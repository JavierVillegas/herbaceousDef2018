...
//        }
    ...
    // filling the tree structure
      ...
      vector<ofVec3f> PosList1;
      vector<ofVec3f> PosList2;
      vector<ofVec3f> PosList3;
        ...
        while ((TheError>2)&&(itThres<5)) {
            ...
            for (int k=0; k < Nob;k++){
                if (TheObjects[k].x<TreeThres1) {
                    ...
                    PosList1.push_back(ofVec3f(TheObjects[k].x,TheObjects[k].y,0.0));
                    sideCounter1++;
                }
                else if((TheObjects[k].x>=TreeThres1)&&(TheObjects[k].x<TreeThres2))
                {
                    cordX2+=TheObjects[k].x;
                    PosList2.push_back(ofVec3f(TheObjects[k].x,TheObjects[k].y,0.0));
                    sideCounter2++;
                }
                else{
                    cordX3+=TheObjects[k].x;
                    PosList3.push_back(ofVec3f(TheObjects[k].x,TheObjects[k].y,0.0));
                    sideCounter3++;
                }
            } // end for 0..Nob
            if (sideCounter1!=0){
      ...
      // creating root nodes
      TheNode RootTree1(0,ofVec2f(cordX1, (Nh*1.65)*.7));
      ...
      LeavesCentroid(&RootTree1, PosList1,0);
      LeavesCentroid(&RootTree2, PosList2,1);
      LeavesCentroid(&RootTree3, PosList3,2);
      
      
//      PosList2 = LeavesCentroid(&RootTree2, PosList2);
//      PosList3 = LeavesCentroid(&RootTree3, PosList3);
...
//      generatVertexArray3(&RootTree3);
      NumLineas[0] = treeVec[0].size();
      NumLineas[1] = treeVec[1].size();
      NumLineas[2] = treeVec[2].size();
...
//      cout<<NumLineas[2]<<endl;
      ...
      // coping to the vbo
      Tree1Vbo.setVertexData(&treeVec[0][0], treeVec[0].size(), GL_DYNAMIC_DRAW);

     // leaves
      Leaves1Vbo.setVertexData(&PosList1[0], PosList1.size(), GL_DYNAMIC_DRAW);
      
      Tree2Vbo.setVertexData(&treeVec[1][0], treeVec[1].size(), GL_DYNAMIC_DRAW);
      
      Leaves2Vbo.setVertexData(&PosList2[0], PosList2.size(), GL_DYNAMIC_DRAW);
      Tree3Vbo.setVertexData(&treeVec[2][0], treeVec[2].size(), GL_DYNAMIC_DRAW);
      Leaves3Vbo.setVertexData(&PosList3[0], PosList3.size(), GL_DYNAMIC_DRAW);
  //    treeVec1.clear();
  //    treeVec2.clear();
  //    treeVec3.clear();
...
void TreeDLA::LeavesCentroid(TheNode* Node, const vector<ofVec3f> &ListaP, int lisInd){
 ...
 //   TheNode RightNode(Node->level+1);
    ...
    vector<ofVec3f> LeftList;
    vector<ofVec3f> RightList;
    ofVec2f meanL(0.0,0.0);
    ofVec2f meanR(0.0,0.0);
    // going through the list
    for(int pt =0 ; pt < ListaP.size();pt++){
   
      // move to the left
      if (ListaP[pt].x < Node->TheMean.x){
        ofVec3f aPoint = ListaP[pt];
        LeftNode->NodeId = aPoint.z;
        LeftList.push_back(ListaP[pt]);
        meanL.x+= ListaP[pt].x;
        meanL.y+= ListaP[pt].y;
      }
      else{
        ofVec3f aPoint = ListaP[pt];
        aPoint.z += pow(2,Node->level);
        RightNode->NodeId = aPoint.z;
        RightList.push_back(ListaP[pt]);
        meanR.x+= ListaP[pt].x;
        meanR.y+= ListaP[pt].y;
      }
   }
    // calculating means
    if (LeftList.size()>0){
      // finding a point at 30% Y distance between the two joins
      float per = 0.3;
      LeftNode->setMean(meanL/(float)LeftList.size());
      float longi = Node->TheJoin.y - LeftNode->TheMean.y;
      LeftNode->setJoin(ofVec2f(LeftNode->TheMean.x,
                                Node->TheJoin.y-per*longi));
      //
      treeVec[lisInd].push_back(ofVec3f(Node->TheJoin.x,
                                 Node->TheJoin.y, Node->level));
      treeVec[lisInd].push_back(ofVec3f(LeftNode->TheJoin.x,
                                 LeftNode->TheJoin.y, Node->level));
    }
    else{
      LeftNode->setMean(Node->TheMean);
      LeftNode->setJoin(Node->TheJoin);
    }
    if (RightList.size()>0){
      float per = 0.3;
      RightNode->setMean(meanR/(float)RightList.size());
      float longi = Node->TheJoin.y - RightNode->TheMean.y;
      RightNode->setJoin(ofVec2f(RightNode->TheMean.x,
                                Node->TheJoin.y-per*longi));
      treeVec[lisInd].push_back(ofVec3f(Node->TheJoin.x,
                                 Node->TheJoin.y, Node->level));
      treeVec[lisInd].push_back(ofVec3f(RightNode->TheJoin.x,
                                 RightNode->TheJoin.y, Node->level));
    }
    else{
      RightNode->setMean(Node->TheMean);
      RightNode->setJoin(Node->TheJoin);
...
//    RightNode.printLeaves("Right");
    LeavesCentroid(LeftNode.get(),LeftList,lisInd);
    LeavesCentroid(RightNode.get(),RightList,lisInd);
    Node->assignLeft(LeftNode.get());
    Node->assignRight(RightNode.get());
    return;
  }
  else if ((Node->level >= MAXTREELEVEL)&&
           (ListaP.size()>0)){
    
    for (int pl = 0; pl < ListaP.size(); pl++){
      treeVec[lisInd].push_back(ofVec3f(Node->TheJoin.x,
                                 Node->TheJoin.y, Node->level));
      treeVec[lisInd].push_back(ofVec3f(ListaP[pl].x,
                                 ListaP[pl].y, Node->level));
